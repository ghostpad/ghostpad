...
import { openModal, updateLoadedFile, updateMovingWIEntry } from "@/store/uiSlice";
import {
  PiFileFill,
} from "react-icons/pi";
    ...
    >
      ...
      <div className="collapse-content px-0 pt-8 border-t border-neutral">
        <div className="flex px-4 mb-4 space-x-2">
          <button
            className="btn btn-sm btn-neutral align-middle flex-1"
          ...
          <div className="dropdown dropdown-bottom dropdown-end flex-1" title="Import or export JSON files containing world info folders.">
            <label tabIndex={1} className="btn btn-sm btn-neutral mb-2 w-full">
              <PiFileFill size="1.5em" /> File
            </label>
            <ul
              tabIndex={0}
            ...
            >
              <li>
                ...
                <button
                  onClick={() => {
                    document.location.href = `//${ghostpadConfig.host}/export_world_info_folder?folder=${folderName}`;
                  }}
                  className="flex flex-row justify-start"
                >
        ...
        <div className="text-center space-x-2 px-4 mb-4">
          <button
            className="btn btn-sm btn-primary px-4 w-3/4"
        ...
        {folderUid !== "-1" && (
          <div className="flex space-x-2 px-4 mt-4">
            <button
              className="btn btn-sm bg-error/40 flex-1"
              onClick={() => {
